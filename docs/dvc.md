...
Amazon Simple Storage Service (S3) for managing the SER dataset. The focus is on speech emotion recognition,
...
including audio files and images like spectrograms. The decision to employ DVC for the dataset management is driven by
...
scalability, cost-effective data administration, and reliable data durability. Storing the dataset in S3
...
~~~yaml
outs:
- md5: 7bd0b5711c6f47e00a027e02023dea1e.dir
  size: 2511827750
  nfiles: 16402
  path: data
~~~
...
In the domain of Speech Emotion Recognition, DVC pipelines emerge as a fundamental element, serving as a structured
...
**Reproducibility**:
DVC pipelines offer a robust mechanism for capturing, versioning, and reproducing the entire Speech Emotion Recognition
...
**Dependency Tracking**:
DVC explicitly defines dependencies between different stages of the Speech Emotion Recognition workflow. This ensures
...
The implemented pipeline, facilitated by the DVC framework, represents the Audio-Based Speech Emotion Recognition
Pipeline detailed in the preceding sub-section. This integration seamlessly combines the feature extraction pipeline and
the model training process, orchestrating a cohesive and reproducible workflow for Speech Emotion Recognition.

**Workflow Structure and Reproducibility**:
By structuring the workflow using DVC pipelines, this thesis ensures a systematic and reproducible approach to ser. 
The clear delineation of stages, version control integration, and dependency tracking collectively
contribute to a robust and transparent ML workflow. The use of DVC pipelines enhances collaboration, facilitates
experimentation, and provides a foundation for credible research findings in the dynamic field of ser.


~~~yaml
stages:
  feature_extraction:
    cmd: python3 __main__.py create-image-features-from-audio --path_with_audios_dir data/ --path_to_image Image_data
    deps:
      - __main__.py
      - data
    outs:
      - Image_data
  train:
    cmd: python3 __main__.py train-using-image-features --model resnet18 --criterion cross_entropy --optimizer sdg --checkpoints_path ser_checkpoints --images_path Image_data
    deps:
      - __main__.py
      - Image_data
    outs:
